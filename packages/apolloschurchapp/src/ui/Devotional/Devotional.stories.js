...
const devotionalTab = () => (
  <ContentContainer>
    <H2 padded isLoading={!songOfSolomon1.devotional.title && songOfSolomon1.loading}>
      {songOfSolomon1.devotional.title}
    </H2>
    {(songOfSolomon1.scripture.reference && songOfSolomon1.scripture.reference.length) ? (
      <ScriptureLink onPress={() => jumpTo('scripture')}>
        {songOfSolomon1.scripture.reference}
      </ScriptureLink>
    ) : null}
    <HTMLView isLoading={!songOfSolomon1.devotional.text && songOfSolomon1.loading}>
      {songOfSolomon1.devotional.text}
    </HTMLView>
  </ContentContainer>
);
