...
CURRENT_VERSION=$(curl http://itunes.apple.com/lookup\?bundleId\=$APP_ID | grep "version" | sed 's/.*"version":"\([0-9\.]*\)".*/\1/')

# Bumps patch numer
# From https://stackoverflow.com/questions/8653126/how-to-increment-version-number-in-a-shell-script
...
if [ "$TRACK" = "internal" ] || [ -z "$TRACK" ]; then
    ...
    fastlane run increment_version_number version_number:"$VERSION" xcodeproj:"ios/$TARGET.xcodeproj"
